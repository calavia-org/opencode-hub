...
- **Org-wide automation** → Archive SPECs across all repos via org workflow
...
- [ ] Bot triggers archive workflow on merge
...
- [ ] Archive logic:
  ...
  3. Check `/.specs/{issue}-{slug}.md` exists
  4. Move to `/.specs/archived/{issue}-{slug}.md`
...
2. Should the org workflow create a PR for archival or attempt direct push first?
   - **Recommendation**: Attempt direct push first, fallback to PR
3. How should we handle SPECs that were never in `/.specs/` (legacy)?
   - **Recommendation**: Skip archive, log warning
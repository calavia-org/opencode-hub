...
**Prerequisites**: Load `core/concepts/spec-driven-process.md`, `core/guides/github-workflow-rules.md`
...
# Add private key to GitHub Secrets (Settings → Secrets and variables → Actions)
# Name: SSH_PRIVATE_KEY_BOT
# Value: (contents of ~/.ssh/my_project_bot)
...
### 5.1 Create Context Structure

```bash
# Copy from opencode-hub (recommended)
...
mkdir -p .opencode/context/ui/web
```

### 5.2 Add Project-Specific Context
...
**Why**: Enforces clean history + `CHANGELOG.md` auto-generation.
...
## Step 9: Add Project to OpenCode Hub (Optional)

If you want your project to be discoverable:

1. Fork `https://github.com/calavia-org/opencode-hub`
2. Add your project to the registry (ask me to help with this)
3. Submit PR
4. After merge, your project is discoverable at `https://opencode.calavia.org/`
...
| Secret | Value |
|---------|-------|
| `OPENCODE_BOT_TOKEN` | Bot's fine-grained PAT |
| `SSH_PRIVATE_KEY_BOT` | Bot's SSH private key |
| `HUMAN_TOKEN` | Your personal token |
...
- [ ] Bot token (`OPENCODE_BOT_TOKEN`) added to secrets
- [ ] Human token (`HUMAN_TOKEN`) added to secrets
...
- **spec-driven Agent**: `spec-driven.md` (in OpenAgentsControl repo)
